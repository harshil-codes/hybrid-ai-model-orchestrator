CREATE OR REPLACE TABLE `loan_data.loan_training_data_raw_v3` AS
WITH entity_agg AS (
  SELECT
    entity_id,
    entity_type,
    ...
    WHEN entity_type = 'business' THEN
      CASE
        WHEN avg_credit_score >= 620
             AND loan_to_income_ratio <= 1 THEN 1 ELSE 0 END
    WHEN entity_type = 'individual' THEN
      CASE
        WHEN avg_credit_score >= 640
             AND loan_to_income_ratio <= 0.85 THEN 1 ELSE 0 END
    ELSE 0
